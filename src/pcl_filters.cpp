...
// pcl_filters.cpp is a toolbox that implements the most commonly used features from PCL.
...
//
#include "../include/agilus_master_project/pcl_filters.hpp"
...
int PclFilters::search_for_model(std::vector<RayTraceCloud> clusters, std::vector<RayTraceCloud> model){
...
void PclFilters::ransac_recognition(std::vector<RayTraceCloud> models, RayTraceCloud object){
...
std::vector<float> PclFilters::match_cloud(RayTraceCloud object_model, pcl::KdTreeFLANN<pcl::VFHSignature308>::Ptr search_tree){
...
std::vector<float> PclFilters::temp_matching_cvfh(RayTraceCloud object_model, pcl::KdTreeFLANN<pcl::VFHSignature308>::Ptr search_tree){
    std::vector<float> returnvalues;
    std::vector<int> best_match(1);
    std::vector<float> square_distance(1);
...
pcl::PointCloud<pcl::PointXYZ>::Ptr PclFilters::calculate_keypoints(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, float min_scale, int nr_octaves, int nr_scales_per_octave, float min_contrast){
...
pcl::PointCloud<pcl::FPFHSignature33>::Ptr PclFilters::calculate_local_descritor(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr normal, pcl::PointCloud<pcl::PointXYZ>::Ptr keypoints, float feature_radius){
...
std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> PclFilters::cluster_extraction(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, double distance){
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_f (new pcl::PointCloud<pcl::PointXYZ>), incloud (new pcl::PointCloud<pcl::PointXYZ>);
...
pcl::PointCloud<pcl::VFHSignature308>::Ptr PclFilters::calculate_ourcvfh_descriptors(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr normal){
    ...
    ourcvfh.setEPSAngleThreshold(5.0 / 180.0 * M_PI);
...
icpResult PclFilters::object_detection(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, std::vector<RayTraceCloud> model_a, std::vector<RayTraceCloud> model_b){
    ...
    *scene_copy += *a_transformed;
    ...
    icpResult result;
    result.cloud = scene_copy;
    result.partAFinal = final_a * model_a.at(result_a.at(0)).pose;
    result.partBFinal = final_b * model_b.at(result_b.at(0)).pose;
    return result;
}
}
